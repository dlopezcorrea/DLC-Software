...
export default function AcceptInvitePage() {
  ...
  async function onSubmit(data: FormData) {
    try {
      setError('');
      await apiClient.post(`/auth/accept-invite/${token}`, { password: data.password });
      setSuccess(true);
      setTimeout(() => navigate('/login'), 2000);
    } catch (e: any) {
      setError(e.response?.data?.error?.message ?? 'Could not accept invite. The link may have expired.');
    }
  }
  ...
  if (!token) {
    return (
      <div className="text-center space-y-4">
        ...
        <Link to="/login" className="text-indigo-600 text-sm hover:underline">Go to login</Link>
      </div>
    );
  }
  ...
  if (success) {
    return (
      <div className="text-center space-y-4">
        ...
        <p className="text-slate-500 text-sm">Your account is ready. Redirecting to login...</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">Set a password to activate your account</p>
      </div>
